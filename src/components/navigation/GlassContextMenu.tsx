...
export const GlassContextMenuContent: React.FC<GlassContextMenuContentProps> = ({
    ...
    return (
        <Motion preset="scaleIn" className="glass-glass-fixed z-[9999]">
            <OptimizedGlass
            intent="neutral"
          elevation="level4"
          intensity="strong"
          ...
          ref={menuRef}
            ...
            >
                <div className="glass-glass-py-1">
                    {items.map((item, index) => (
                        <React.Fragment key={item?.id}>
                            ...
                            {item?.children && submenuOpen === item?.id && (
                                <div className="glass-glass-absolute left-full top-0 glass-ml-1">
                                    <GlassContextMenuContent
                                        items={item?.children}
                                        position={{ x: 0, y: 0 }}
...
export const GlassContextMenuItem: React.FC<GlassContextMenuItemProps> = ({
    ...
    if (item?.separator) {
        return <div className="h-px glass-surface-subtle/20 glass-glass-mx-2 glass-glass-my-1" />;
    }
    ...
    return (
        ...
        >
            {item?.icon && (
                <div className="glass-glass-flex glass-glass-items-center glass-glass-justify-center glass-glass-w-4 glass-glass-h-4 mr-3">
                    {item?.icon}
                </div>
            )}

            <span className="glass-glass-flex-1 glass-glass-text-sm glass-glass-font-medium glass-glass-truncate">
                {item?.label}
            </span>

            {item?.shortcut && (
                <span className="ml-6 glass-glass-text-xs glass-glass-text-primary/50 font-mono">
                    {item?.shortcut}
                </span>
            )}

            {item?.children && (
                <div className="ml-3 glass-glass-text-primary/50">
                    ▶
                </div>
            )}
        </div>
