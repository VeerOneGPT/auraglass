...
export const GlassFileUpload: React.FC<GlassFileUploadProps> = ({
  ...
  const renderFilePreview = (uploadedFile: UploadedFile) => {
    ...
    return (
      <div key={uploadedFile.id} className="glass-glass-flex glass-glass-items-center glass-glass-p-3 bg-muted glass-radius-lg glass-glass-border">
        <div className="glass-glass-text-2xl mr-3">{getFileIcon(file)}</div>
        ...
        <div className="glass-glass-flex-1 glass-glass-min-w-0">
          <div className="glass-glass-flex glass-glass-items-center glass-glass-justify-between">
            <p className="glass-glass-text-sm glass-glass-font-medium glass-glass-text-primary glass-glass-truncate">{file.name}</p>
            <button
              onClick={() => removeFile(uploadedFile.id)}
              className="ml-2 glass-glass-text-primary hover:glass-glass-text-primary glass-glass-text-sm"
          ...
          <p className="glass-glass-text-xs glass-text-secondary">
            ...
            {status === 'completed' && <span className="glass-glass-text-primary ml-2">✓ Completed</span>}
            {status === 'error' && <span className="glass-glass-text-primary ml-2">✗ {error}</span>}
          </p>
          
          {status === 'uploading' && (
            <div className="mt-2">
              <div className="glass-glass-w-full bg-gray-200 glass-radius-full h-1">
                <div 
                  className="glass-surface-blue h-1 glass-radius-full transition-all duration-300"
                  style={{ width: `${progress}%` }}
                />
              </div>
              <p className="glass-glass-text-xs glass-text-secondary mt-1">{progress}%</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className={cn('w-full', className)} {...props}>
      ...
      >
        <div className="glass-glass-flex glass-glass-flex-col glass-glass-items-center glass-glass-gap-4">
          <div className="glass-glass-text-4xl opacity-60">
            {isUploading ? '⏳' : '📁'}
          </div>
          
          <div>
            <p className="glass-glass-text-lg glass-glass-font-medium glass-glass-text-primary glass-glass-mb-2">
          ...
          {!isUploading && (
            <button
              type="button"
              className="glass-glass-px-4 glass-glass-py-2 glass-surface-primary glass-glass-text-primary-foreground glass-radius-lg hover:glass-surface-primary/90 transition-colors glass-glass-font-medium"
              disabled={disabled}
            >
              {browseText}
            </button>
      ...
      {showPreview && files.length > 0 && (
        <Glass className="glass-glass-border-t-0 glass-radius-b-xl glass-glass-p-4 glass-glass-space-y-3">
          <h4 className="glass-glass-font-medium glass-glass-text-primary glass-glass-mb-3">
