...
const meta: Meta<typeof ParticleBackground> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "Animated particle background shown behind readable media content.",
      },
    },
  },
  argTypes: {
...
export const Default: Story = {
  render: (args) => (
    ...
    >
      <div
        className="glass-w-full glass-max-w-3xl glass-rounded-2xl glass-p-8 glass-shadow-2xl glass-backdrop-blur-md glass-border glass-border-subtle"
        style={{
      ...
      >
        <p
          className="glass-text-sm glass-font-semibold glass-uppercase glass-tracking-wide"
          style={{ color: "rgba(20,25,32,.64)" }}
        >
          Background layer
        </p>
        <h1
        ...
        >
          {["Nodes", "Throughput", "Noise"].map((label, index) => (
            <div
              key={label}
              className="glass-rounded-xl glass-p-4 glass-border glass-border-subtle"
              style={{ background: "rgba(255,255,255,.46)" }}
            >
