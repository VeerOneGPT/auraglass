    ...
    if (orientation === 'horizontal') {
      return (
        ...
        >
          {showLine && (
            <div
              className="glass-glass-absolute top-8 left-0 right-0 h-px bg-glass-glass-border/20"
              style={{ backgroundColor: lineColor }}
            />
          )}
          {items.map((item, index) => (
            <div key={item.id} className="glass-glass-relative glass-glass-flex-shrink-0 glass-glass-min-w-0" role="listitem">
              {/* Dot */}
              <div
                className={cn(
      ...
      >
        ...
        <ul className={cn('relative', config.gap)} role="list">
          {items.map((item, index) => {
            ...
            return (
              <li key={item.id} className="glass-glass-relative" role="listitem">
                {/* Dot */}
                <span
                  className={cn(
                ...
                >
                  <div className="glass-glass-flex glass-glass-items-start glass-glass-justify-between glass-glass-gap-4">
                    <div className="glass-glass-flex glass-glass-items-start glass-glass-gap-3 glass-glass-min-w-0 glass-glass-flex-1">
                      {item.icon && (
                        <div className="glass-glass-flex-shrink-0 glass-text-secondary glass-mt-0-5">
                          {item.icon}
                        </div>
                      )}
                      <div className="glass-glass-min-w-0 glass-glass-flex-1">
    ...
    return (
      <div ref={ref} className={cn('timeline-item', className)} {...props}>
        ...
        <OptimizedGlass
          ...
          animation="none"
          performanceMode="medium"
          className="glass-radius-lg glass-glass-p-3"
        >
          <div className="glass-glass-font-medium glass-glass-text-primary">{item.title}</div>
          {item.subtitle && (
            <div className="glass-glass-text-sm glass-text-secondary glass-mt-1">{item.subtitle}</div>
          )}
          {item.time && (
            <div className="glass-glass-text-xs glass-text-secondary glass-mt-2">{item.time}</div>
          )}
        </OptimizedGlass>
      </div>
    );
