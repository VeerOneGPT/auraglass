    ...
    <div key={c.id} className="space-y-2">
      ...
      <div className="ml-6">
        <div className="flex gap-2 items-center">
          <input value={drafts[c.id] ?? ''} onChange={(e)=>setDraft(c.id, e.target.value)} placeholder="Reply…" className="flex-1 bg-transparent border border-white/20 rounded-lg px-2 py-1 text-sm outline-none" />
