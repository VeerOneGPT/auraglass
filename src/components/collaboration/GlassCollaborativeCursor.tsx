...
const CursorIcon: React.FC<{ color: string; name: string }> = ({ color, name }) => (
  <div className="glass-glass-relative">
    ...
    {/* User name label */}
    <div
      className="glass-glass-absolute left-6 glass--glass-top-1 glass-glass-px-2 glass-glass-py-1 glass-radius glass-glass-text-primary glass-glass-text-xs glass-glass-font-medium whitespace-nowrap glass-pointer-events-none select-none"
    ...
    <div className={cn("fixed inset-0 pointer-events-none z-50", className)}>
      {visibleCursors.map(user => {
        ...
        return (
          <div
            key={user.id}
            className="glass-glass-absolute transition-all duration-100 ease-out"
            style={{
              left: user.cursor.x,
              top: user.cursor.y,
