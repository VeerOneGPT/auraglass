  ...
  (
    ...
    const renderBarChart = () => {
      if (!data?.dataPoints || data.dataPoints.length === 0) {
        return <div className="glass-glass-flex glass-glass-items-center glass-glass-justify-center glass-glass-h-full glass-text-secondary">No data available</div>;
      }

      const maxValue = Math.max(...data.dataPoints.map(d => d.value));
      
      return (
        <div className="glass-glass-flex glass-glass-items-end glass-glass-justify-between glass-glass-gap-2 glass-glass-h-full">
          {data?.dataPoints.map((point, index) => {
            ...
            return (
              <Motion
                ...
                preset="slideUp"
                delay={index * 100}
                className="glass-glass-flex-1 glass-glass-flex glass-glass-flex-col glass-glass-items-center glass-glass-gap-2"
              >
                <div
                  className="glass-glass-w-full glass-radius-t transition-all duration-300 hover:opacity-80 glass-glass-cursor-pointer"
                  style={{
                    height: `${height}%`,
                ...
                />
                <div className="glass-glass-text-xs glass-text-secondary glass-glass-text-center glass-glass-truncate glass-glass-w-full">
                  {point.label}
                </div>
              </Motion>
            );
    ...
    const renderLineChart = () => {
      ...
      return (
        <div className="glass-glass-relative glass-glass-h-full glass-glass-w-full">
          <svg className="glass-glass-w-full glass-glass-h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            {/* Grid lines */}
            {showGrid && (
              <g>
            ...
            <polyline
              ...
              strokeWidth="2"
              className="drop-glass-glass-shadow-sm"
            />
            ...
            {data?.dataPoints.map((point, index) => {
              ...
              return (
                <circle
                  ...
                  className="hover:r-3 transition-all glass-glass-cursor-pointer"
                />
              );
            })}
          </svg>
          ...
          {/* Labels */}
          <div className="glass-glass-absolute bottom-0 left-0 right-0 glass-glass-flex glass-glass-justify-between">
            {data?.dataPoints.map((point, index) => (
              <div key={index} className="glass-glass-text-xs glass-glass-text-primary/80">
                {point.label}
              </div>
            ))}
          </div>
              ...
              {data?.dataPoints.map((point, index) => {
                ...
                return (
                  <path
                    ...
                    d={pathData}
                    fill={color}
                    className="hover:opacity-80 transition-opacity glass-glass-cursor-pointer"
                  />
                );
              })}
            </svg>
    ...
    const renderSparkline = () => {
      if (!data?.dataPoints || data.dataPoints.length === 0) {
        return <div className="glass-glass-flex glass-glass-items-center glass-glass-justify-center glass-glass-h-full glass-text-secondary glass-glass-text-xs">No data</div>;
      }
      ...
      return (
        <svg className="glass-glass-w-full glass-glass-h-8" viewBox="0 0 100 100" preserveAspectRatio="none">
          <polyline
            points={points}
            fill="none"
            stroke={colors?.[0] || '#3B82F6'}
    ...
    const renderChartContent = () => {
      if (loading) {
        return (
          <div className="glass-glass-flex glass-glass-items-center glass-glass-justify-center glass-glass-h-full">
            <div className="glass-glass-w-6 glass-glass-h-6 glass-glass-border-2 glass-glass-border-primary glass-glass-border-t-transparent glass-radius-full animate-spin" />
          </div>
        );
      }

      if (!data) {
        return <div className="glass-glass-flex glass-glass-items-center glass-glass-justify-center glass-glass-h-full glass-text-secondary">No chart data provided</div>;
      }
      ...
      return (
        <div className="glass-glass-flex glass-glass-flex-wrap glass-glass-gap-2">
          {data.dataPoints.map((point, index) => {
            const color = point.color || colors[index % (colors?.length || 0)];
            return (
              <div key={index} className="glass-glass-flex glass-glass-items-center glass-glass-gap-1">
                <div
    ...
    return (
      <Glass
        ...
        {...props}
      >
        <VStack space="md" className="glass-glass-h-full">
          {/* Header */}
          ...
          {data?.summary && variant !== 'minimal' && (
            <HStack space="sm" align="center">
              {data?.summary.total && (
                <div className="glass-glass-text-lg font-bold glass-glass-text-primary">
