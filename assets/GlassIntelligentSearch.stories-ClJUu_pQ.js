import{r as p,a as ve,j as e,c as R,R as Y}from"./iframe-C5od7h8K.js";import{G as V}from"./GlassCore-D7CVB_xb.js";import"./preload-helper-PPVm8Dsz.js";const X=r=>Array.isArray(r)&&r.every(n=>typeof n=="string"),Se=r=>typeof r=="object"&&r!==null&&!Array.isArray(r)&&"start"in r&&"end"in r,M=r=>X(r)?r:[],oe=(r,n)=>typeof r=="number"?r:n,ge=r=>{const n=r.toLowerCase();let m="search";n.includes("find")||n.includes("show")||n.includes("get")?m="retrieve":n.includes("compare")||n.includes("vs")||n.includes("versus")?m="compare":n.includes("best")||n.includes("top")||n.includes("recommend")?m="recommend":(n.includes("help")||n.includes("how to")||n.includes("tutorial"))&&(m="help");const c=[];r.match(/(\d{4}|\d{1,2}\/\d{1,2}\/\d{4}|today|yesterday|last week|this month)/gi)?.forEach(a=>{c.push({type:"date",value:a})}),r.match(/\b\d+(?:\.\d+)?\b/g)?.forEach(a=>{c.push({type:"number",value:a})}),["product","service","article","document","image","video","user","project"].forEach(a=>{n.includes(a)&&c.push({type:"category",value:a})});const d=["good","great","excellent","amazing","best","love","fantastic"],i=["bad","terrible","awful","worst","hate","horrible","useless"],h=d.some(a=>n.includes(a)),S=i.some(a=>n.includes(a));let u="neutral";h&&!S?u="positive":S&&!h&&(u="negative");const j=["the","a","an","and","or","but","in","on","at","to","for","of","with","by","is","are","was","were","been","be","have","has","had","do","does","did","will","would","could","should","may","might","can","this","that","these","those"],f=r.toLowerCase().split(/\s+/).filter(a=>a.length>2&&!j.includes(a)).filter((a,y,N)=>N.indexOf(a)===y);return{intent:m,entities:c,sentiment:u,keywords:f}},je=(r,n,m,c=!0)=>{if(!r.trim()&&Object.keys(m).length===0)return n;const x=c?ge(r):null,g=r.toLowerCase().split(/\s+/).filter(l=>l.length>0);return n.map(l=>{let d=0;const i=g.reduce((f,a)=>f+(l.title.toLowerCase().includes(a)?1:0),0),h=g.reduce((f,a)=>f+(l.description.toLowerCase().includes(a)?.5:0),0),S=g.reduce((f,a)=>f+l.tags.filter(y=>y.toLowerCase().includes(a)).length*.7,0);if(d=i*3+h+S,x&&c){x.intent==="recommend"&&l.metadata?.rating&&(d+=l.metadata.rating*.5),x.entities.forEach(a=>{a.type==="category"&&l.category.toLowerCase().includes(a.value)&&(d+=1)});const f=x.keywords.filter(a=>l.title.toLowerCase().includes(a)||l.description.toLowerCase().includes(a)).length;d+=f*.3}let u=!0;Object.entries(m).forEach(([f,a])=>{if(!(!a||Array.isArray(a)&&a.length===0))switch(f){case"category":X(a)?u=u&&a.includes(l.category):typeof a=="string"&&(u=u&&l.category===a);break;case"tags":X(a)&&(u=u&&a.some(y=>l.tags.includes(y)));break;case"dateRange":if(l.metadata?.date&&Se(a)){const y=new Date(l.metadata.date),N=new Date(a.start),F=new Date(a.end);u=u&&y>=N&&y<=F}break;case"rating":l.metadata?.rating&&typeof a=="number"&&(u=u&&l.metadata.rating>=a);break}}),u||(d=0);const j={};return d>0&&g.forEach(f=>{l.title.toLowerCase().includes(f)&&(j.title||(j.title=[]),j.title.push(f)),l.description.toLowerCase().includes(f)&&(j.description||(j.description=[]),j.description.push(f))}),{...l,score:d,highlights:j}}).filter(l=>l.score>0).sort((l,d)=>d.score-l.score)},ce=(r,n,m)=>{const c=[];if(!n||n.length===0)return c;const x=r.toLowerCase();m.filter(i=>i.toLowerCase().includes(x)).slice(0,3).forEach(i=>{c.push({text:i,type:"query"})}),[...new Set(n.map(i=>i.category))].filter(i=>i.toLowerCase().includes(x)).slice(0,3).forEach(i=>{const h=n.filter(S=>S.category===i).length;c.push({text:i,type:"category",category:"Categories",count:h})});const d=n.flatMap(i=>i.tags).reduce((i,h)=>(i[h]=(i[h]||0)+1,i),{});return Object.entries(d).filter(([i])=>i.toLowerCase().includes(x)).sort(([,i],[,h])=>h-i).slice(0,5).forEach(([i,h])=>{c.push({text:i,type:"tag",category:"Tags",count:h})}),c},$=({data:r=[],initialQuery:n="",onSearch:m,onResultClick:c,placeholder:x="Search with natural language...",showFilters:g=!0,showSuggestions:l=!0,suggestionsInitiallyOpen:d=!1,enableNLP:i=!0,enableVoiceSearch:h=!1,maxResults:S=50,className:u,"aria-label":j,"data-testid":f})=>{const[a,y]=p.useState(n),[N,F]=p.useState([]),[K,Z]=p.useState(()=>n?ce(n,r,[]):[]),[w,I]=p.useState({}),[de,T]=p.useState(d),[U,E]=p.useState(!1),[ee,pe]=p.useState([]),[A,he]=p.useState(null),[se,G]=p.useState(!1),ae=p.useRef(null),C=p.useRef(),ue=p.useRef([]),D=p.useRef(!1),P=p.useRef(!0),q=ve(),te=p.useMemo(()=>{if(!r||r.length===0)return[];const s=[...new Set(r.map(o=>o.category))],b=r.flatMap(o=>o?.tags||[]).reduce((o,v)=>(v&&(o[v]=(o[v]||0)+1),o),{}),k=Object.entries(b).sort(([,o],[,v])=>v-o).slice(0,20).map(([o,v])=>({value:o,label:o,count:v}));return[{id:"category",name:"Category",type:"multiselect",options:s.map(o=>({value:o,label:o,count:r.filter(v=>v.category===o).length}))},{id:"tags",name:"Tags",type:"multiselect",options:k},{id:"rating",name:"Minimum Rating",type:"range",range:{min:0,max:5,step:.5}}]},[r]);p.useEffect(()=>(D.current=!0,P.current=!0,()=>{D.current=!1,C.current&&clearTimeout(C.current)}),[]);const re=p.useCallback((s,t)=>{D.current&&(E(!0),C.current&&clearTimeout(C.current),C.current=setTimeout(()=>{if(!D.current)return;if(!s.trim()&&Object.keys(t).length===0){F([]),E(!1);return}const k=je(s,r,t,i).slice(0,S);F(k),E(!1),i&&s.trim()&&he(ge(s)),m?.(s,t)},300))},[r,i,S,m]);p.useEffect(()=>{if(P.current){P.current=!1;return}if(D.current)if(a.length>0){const s=r.length>0?r:[],t=ce(a,s,ee);Z(t),T(!0)}else Z([]),T(!1)},[a]);const ne=p.useRef(re);ne.current=re,p.useEffect(()=>{if(P.current){P.current=!1;return}if(D.current){if(!a.trim()&&Object.keys(w).length===0){F([]),E(!1);return}return ne.current(a,w),()=>{C.current&&clearTimeout(C.current)}}},[a,w]);const me=s=>{y(s)},fe=s=>{s.type==="query"?y(s.text):s.type==="category"?I(t=>({...t,category:[...M(t.category),s.text]})):s.type==="tag"&&I(t=>({...t,tags:[...M(t.tags),s.text]})),T(!1),ae.current?.focus()},ie=()=>{a.trim()&&!ee.includes(a.trim())&&pe(s=>[a.trim(),...s.slice(0,9)]),T(!1)},H=(s,t)=>{I(b=>({...b,[s]:t}))},xe=()=>{I({})},be=()=>{const s=window.webkitSpeechRecognition;if(!h||!s)return;const t=new s;t.lang="en-US",t.continuous=!1,t.interimResults=!1,t.onstart=()=>{G(!0)},t.onresult=b=>{const k=b.results[0][0].transcript;y(k),G(!1)},t.onerror=()=>{G(!1)},t.onend=()=>{G(!1)},t.start()},le=(s,t=[])=>{if(!t.length)return s;const b=t.filter(Boolean).map(o=>o.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"));if(!b.length)return s;const k=new RegExp(`(${b.join("|")})`,"gi");return e.jsx(e.Fragment,{children:s.split(k).map((o,v)=>b.some(we=>new RegExp(`^${we}$`,"i").test(o))?e.jsx("mark",{className:"glass-px-1 glass-radius-sm glass-surface-subtle glass-text-primary",children:o},`${o}-${v}`):e.jsx(Y.Fragment,{children:o},`${o}-${v}`))})},ye=s=>({document:"📄",article:"📝",image:"🖼️",video:"🎥",product:"🛍️",user:"👤",project:"📁",service:"⚙️"})[s.toLowerCase()]||"🔍";return e.jsxs("div",{"data-glass-component":!0,className:R("glass-intelligent-search glass-w-full glass-min-w-0",u),style:{width:"100%",maxWidth:"64rem",margin:"0 auto"},"aria-label":j,"data-testid":f,children:[e.jsx("style",{children:`
