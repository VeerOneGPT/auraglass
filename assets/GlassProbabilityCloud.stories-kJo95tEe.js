import{f as oe}from"./index-DdjpOZjl.js";import{r as p,a as we,j as t,c as r,d as Fe,m as ue}from"./iframe-D7NmxSe9.js";import{u as qe}from"./a11y-AzHiXVvX.js";import{u as je}from"./useMotionPreference-2-Zuo_1E.js";import{c as le}from"./createGlassStyle-Cr0Un8y6.js";import{O as Te}from"./OptimizedGlassCore-KF10QAKi.js";import"./index-ByImX2pa.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-BxFGwbZv.js";const j=(n=.1)=>(Math.random()-.5)*n*2,T=(n,i,P,v=1)=>Math.sin(n*v+P)*Math.cos(i*v+P*.7),ce=n=>n*n,K=p.forwardRef(({width:n=600,height:i=400,depth:P=200,probabilityPoints:v=[],uncertaintyPrinciple:Q=!0,quantumFluctuations:ee=!0,observerEffect:M=!0,heisenbergUncertainty:f=.5,waveParticleDuality:w=!0,showProbabilityDensity:te=!0,showWaveFunction:ae=!0,showUncertaintyBounds:ne=!0,animationSpeed:se=1,particleCount:re=100,measurementPrecision:Ce=.1,realTimeMode:H=!1,onMeasurement:me,onUncertaintyChange:ie,className:pe="",...ye},he)=>{const X=we(),Y=p.useRef(null),[b,ge]=p.useState(0),[F,fe]=p.useState([]),[g,Z]=p.useState([]),[q,be]=p.useState(f);je();const[x,J]=p.useState(!1);qe("glass-probability-cloud"),p.useEffect(()=>{const s=Array.from({length:re},(a,e)=>({id:`particle-${e}`,x:Math.random()*n,y:Math.random()*i,z:Math.random()*P,probability:Math.random(),uncertainty:f+j(.1),waveFunction:Math.random()*2-1,phase:Math.random()*Math.PI*2,observationCount:0}));Z([...v,...s])},[re,n,i,P,f,v]),p.useEffect(()=>{const s=setInterval(()=>{ge(a=>a+.1*se)},16);return()=>clearInterval(s)},[se]),p.useEffect(()=>{if(!H)return;const s=setInterval(()=>{Z(a=>a.map(e=>{const o=T(e.x/n*4*Math.PI,e.y/i*4*Math.PI,b,1+e.phase/Math.PI),c=ce(o);let u=0,l=0,m=0;if(ee){const h=e.uncertainty*.5;u=j(h),l=j(h),m=j(h)}let d=e.uncertainty;if(Q){const Me=1-Math.sqrt(u*u+l*l+m*m);d=Math.max(f,1/(Me+.1))}return M&&x&&Math.sqrt(Math.pow(e.x-n/2,2)+Math.pow(e.y-i/2,2))<50?{...e,waveFunction:Math.sign(o)*.9,probability:.8,uncertainty:Math.max(.1,e.uncertainty*.5),observationCount:e.observationCount+1,lastObserved:Date.now()}:{...e,x:Math.max(0,Math.min(n,e.x+u)),y:Math.max(0,Math.min(i,e.y+l)),z:Math.max(0,Math.min(P,e.z+m)),waveFunction:o,probability:Math.max(.01,Math.min(1,c+.1)),uncertainty:Math.max(.05,Math.min(2,d)),phase:(e.phase+.05)%(2*Math.PI)}}))},50);return()=>clearInterval(s)},[H,b,n,i,P,Q,ee,M,f,x]),p.useEffect(()=>{const s=g.reduce((a,e)=>a+e.uncertainty,0)/g.length;be(s||f),ie?.(s)},[g,f,ie]),p.useEffect(()=>{const s=Y.current;if(!s)return;const a=s.getContext("2d");if(a){if(a.clearRect(0,0,n,i),te){const e=a.createImageData(n,i);for(let o=0;o<n;o+=4)for(let c=0;c<i;c+=4){const u=T(o/n*4*Math.PI,c/i*4*Math.PI,b),l=ce(u),m=Math.floor(l*50+10),d=(c*n+o)*4;d<e.data.length-3&&(e.data[d]=100+m,e.data[d+1]=50+m,e.data[d+2]=200+m,e.data[d+3]=30)}a.putImageData(e,0,0)}if(g.forEach(e=>{const o=M&&e.lastObserved&&Date.now()-e.lastObserved<1e3?.9:e.probability*.7+.1,c=2+e.uncertainty*3,u=w?c*(1+Math.sin(b*3+e.phase)*.3):c;if(w&&e.uncertainty>.3){a.beginPath(),a.arc(e.x,e.y,u*2,0,2*Math.PI),a.fillStyle=`rgba(100, 150, 255, ${o*.3})`,a.fill();for(let l=1;l<=3;l++)a.beginPath(),a.arc(e.x,e.y,u*l*.7,0,2*Math.PI),a.strokeStyle=`rgba(150, 200, 255, ${o*.2/l})`,a.lineWidth=1,a.stroke()}else{a.beginPath(),a.arc(e.x,e.y,u,0,2*Math.PI);const l=a.createRadialGradient(e.x,e.y,0,e.x,e.y,u);l.addColorStop(0,`rgba(255, 255, 255, ${o})`),l.addColorStop(1,`rgba(100, 150, 255, ${o*.3})`),a.fillStyle=l,a.fill()}ne&&e.uncertainty>.2&&(a.beginPath(),a.arc(e.x,e.y,e.uncertainty*20,0,2*Math.PI),a.strokeStyle=`rgba(255, 100, 100, ${e.uncertainty*.3})`,a.lineWidth=1,a.setLineDash([2,3]),a.stroke(),a.setLineDash([]))}),ae){a.strokeStyle="rgba(200, 200, 255, 0.4)",a.lineWidth=1,a.beginPath();for(let e=0;e<n;e+=10){const o=T(e/n*4*Math.PI,i/2/i*4*Math.PI,b),c=i/2+o*30;e===0?a.moveTo(e,c):a.lineTo(e,c)}a.stroke(),a.beginPath();for(let e=0;e<i;e+=10){const o=T(n/2/n*4*Math.PI,e/i*4*Math.PI,b),c=n/2+o*30;e===0?a.moveTo(c,e):a.lineTo(c,e)}a.stroke()}}},[g,b,n,i,te,ae,ne,w,M]);const Pe=s=>{const a=Y.current;if(!a)return;const e=a.getBoundingClientRect(),o=s.clientX-e.left,c=s.clientY-e.top;let u=null,l=1/0;if(g.forEach(m=>{const d=Math.sqrt(Math.pow(m.x-o,2)+Math.pow(m.y-c,2));d<l&&d<30&&(l=d,u=m)}),u){const m={point:u,timestamp:Date.now(),uncertainty:u.uncertainty};fe(d=>[...d.slice(-9),m]),me?.(u),Z(d=>d.map(h=>h.id===u.id?{...h,observationCount:h.observationCount+1,lastObserved:Date.now(),uncertainty:Math.max(.1,h.uncertainty*.7)}:h))}},ve=p.useMemo(()=>g.reduce((s,a)=>s+a.probability,0)/g.length||0,[g]),xe=p.useMemo(()=>1-(q-f),[q,f]);return t.jsx(Te,{ref:he,variant:"frosted",className:`relative ${pe}`,...ye,children:t.jsxs("div",{className:"glass-p-4 glass-space-y-4",children:[t.jsxs("div",{className:"glass-flex glass-items-start glass-justify-between glass-flex-wrap glass-gap-3",children:[t.jsxs("div",{children:[t.jsx("h2",{className:r("glass-text-xl glass-font-semibold glass-text-primary"),children:"Probability Cloud"}),t.jsxs("p",{className:r("glass-text-sm glass-text-secondary"),children:[g.length," quantum states"]})]}),t.jsxs("div",{className:"glass-flex glass-items-center glass-flex-wrap glass-gap-3",children:[t.jsx("button",{onClick:()=>J(!x),className:`
