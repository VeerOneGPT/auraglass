import{r as g,a as Ze,b as T,j as e,m as j}from"./iframe-D7NmxSe9.js";import{u as Je}from"./useMotionPreference-2-Zuo_1E.js";import{u as Ke}from"./a11y-AzHiXVvX.js";import{c as Ge}from"./createGlassStyle-Cr0Un8y6.js";import{u as Xe}from"./soundDesign-C6wKSzTW.js";import{O as Ye}from"./OptimizedGlassCore-KF10QAKi.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-BxFGwbZv.js";const ea=[{id:"stylegan2-faces",name:"StyleGAN2 Faces",description:"High-quality human face generation",type:"stylegan",category:"faces",resolution:1024,latentDim:512,trained:!0},{id:"dcgan-art",name:"DCGAN Art",description:"Abstract art generation",type:"dcgan",category:"art",resolution:256,latentDim:100,trained:!0},{id:"biggan-objects",name:"BigGAN Objects",description:"Conditional object generation",type:"biggan",category:"objects",resolution:512,latentDim:128,trained:!0},{id:"cyclegan-style",name:"CycleGAN Style Transfer",description:"Unpaired image-to-image translation",type:"cyclegan",category:"style_transfer",resolution:256,latentDim:256,trained:!0},{id:"progressive-landscapes",name:"Progressive Landscapes",description:"Landscape scene generation",type:"progressive",category:"landscapes",resolution:512,latentDim:256,trained:!0},{id:"stylegan3-general",name:"StyleGAN3 General",description:"General purpose image generation",type:"stylegan",category:"general",resolution:512,latentDim:512,trained:!1}],aa={seed:42,truncation:.7,styleStrength:1,noiseStrength:.5,batchSize:4,interpolationSteps:10},sa={epochs:100,batchSize:32,learningRate:2e-4,beta1:.5,beta2:.999,discriminatorSteps:1,generatorSteps:1},ta={color:"var(--glass-theme-text, var(--glass-text-primary))"},ne=g.forwardRef(({availableModels:s=ea,selectedModel:Ne="stylegan2-faces",generationParams:je={},trainingConfig:Ce={},compact:y=!1,showHeader:Ae=!y,showActions:Pe=!y,showModelSelector:Ie=!0,showGenerationControls:Le=!1,showTrainingControls:de=!1,showLatentSpace:Te=!1,showProgress:ze=!0,enableInterpolation:ue=!0,enableRealTime:re=!1,maxGenerations:A=16,canvasWidth:na=256,canvasHeight:ra=256,onModelSelect:me,onGenerate:pe,onTrainingProgress:he,className:ke="",...De},Re)=>{const oe=Ze(),[N,fe]=g.useState(!1),[P,be]=g.useState(!1),[C,He]=g.useState(Ne),[le,qe]=g.useState([]),[x,Ve]=g.useState([]),[ie,We]=g.useState({epoch:0,generatorLoss:0,discriminatorLoss:0}),[ye,Fe]=g.useState([]),[xe,ce]=g.useState(0),[d,I]=g.useState({...aa,...je}),[we,oa]=g.useState({...sa,...Ce});g.useRef([]);const Se=g.useRef(null);g.useRef(null),Ke("glass-gan-generator");const{shouldAnimate:M}=Je(),{play:v}=Xe(),h=s.find(a=>a.id===C)||s[0],Me=g.useCallback(a=>Array.from({length:a},()=>(Math.random()-.5)*2),[]),L=g.useCallback((a,t,u)=>{const r=document.createElement("canvas");r.width=t.resolution,r.height=t.resolution;const p=r.getContext("2d");if(!p)return"";const S=p.createImageData(r.width,r.height),n=S.data;switch(t.type){case"stylegan":for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,f=a.slice(0,8).reduce((m,w)=>m+w,0)/8,b=(l/r.width-.5)*2,G=(c/r.height-.5)*2;if(t.category==="faces"){const m=Math.exp(-(b*b+G*G*.8)*2),w=Math.sin(b*10)*Math.cos(G*10)*.1;n[o]=Math.floor((.8+f*.2+w)*m*255),n[o+1]=Math.floor((.7+f*.3+w)*m*255),n[o+2]=Math.floor((.6+f*.4+w)*m*255),n[o+3]=255}else{const m=Math.sin(b*5+f)*Math.cos(G*5+f);n[o]=Math.floor((m+1)*128),n[o+1]=Math.floor((Math.sin(m*2)+1)*128),n[o+2]=Math.floor((Math.cos(m*3)+1)*128),n[o+3]=255}}break;case"dcgan":for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,b=.02*(1+a.slice(0,5).reduce((Ee,Qe)=>Ee+Qe,0)*.1),G=Math.sin(l*b)*Math.cos(c*b),m=Math.sin((l+c)*b*.8),w=Math.cos((l-c)*b*1.2);n[o]=Math.floor((G+1)*128),n[o+1]=Math.floor((m+1)*128),n[o+2]=Math.floor((w+1)*128),n[o+3]=255}break;case"biggan":for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,f=a.slice(-10),b=Math.floor(Math.abs(f[0])*10),m=[[255,100,100],[100,255,100],[100,100,255],[255,255,100],[255,100,255],[100,255,255],[200,150,100],[150,100,200],[100,200,150],[180,180,180]][b]||[128,128,128],w=(Math.random()-.5)*50;n[o]=Math.max(0,Math.min(255,m[0]+w)),n[o+1]=Math.max(0,Math.min(255,m[1]+w)),n[o+2]=Math.max(0,Math.min(255,m[2]+w)),n[o+3]=255}break;default:for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,f=a.slice(0,3).reduce((b,G,m)=>b+G*Math.sin((l+c)*.01*(m+1)),0);n[o]=Math.floor((Math.sin(f)+1)*128),n[o+1]=Math.floor((Math.cos(f*1.1)+1)*128),n[o+2]=Math.floor((Math.sin(f*.9)+1)*128),n[o+3]=255}}return p.putImageData(S,0,0),r.toDataURL()},[]),ge=g.useCallback(async()=>{if(!h.trained){v("error");return}fe(!0),ce(0),v("processing");const a=[],t=[];for(let u=0;u<d.batchSize;u++){ce(u/d.batchSize*100);const p=Me(h.latentDim).map(n=>n*d.truncation),S=L(p,h,d.seed+u);a.push(S),t.push(p),await new Promise(n=>setTimeout(n,T.DURATION.fast*1.3))}qe(u=>[...a,...u].slice(0,A)),Ve(u=>[...t,...u].slice(0,A)),ce(100),pe?.(a,d),fe(!1),v("success")},[h,d,Me,L,A,pe,v]),$e=g.useCallback(async()=>{if(x.length<2)return;const a=x[0],t=x[1],u=d.interpolationSteps,r=[];for(let p=0;p<=u;p++){const S=p/u,n=a.map((l,o)=>l*(1-S)+t[o]*S),c=L(n,h,d.seed);r.push(c)}Fe(r)},[x,d.interpolationSteps,d.seed,L,h]),Ue=g.useCallback(async()=>{if(!h)return;be(!0),v("processing");for(let t=0;t<we.epochs;t++){const u=1+Math.random()*.5-t*.01,r=.8+Math.random()*.3-t*.005;We({epoch:t+1,generatorLoss:Math.max(.1,u),discriminatorLoss:Math.max(.1,r)}),he?.(t+1,{generator:u,discriminator:r}),await new Promise(p=>setTimeout(p,T.DURATION.fast))}const a=s.findIndex(t=>t.id===C);a!==-1&&(s[a].trained=!0),be(!1),v("success")},[h,we.epochs,C,s,he,v]),ve=g.useCallback(()=>{const a=Se.current;if(!a||x.length===0)return;const t=a.getContext("2d");t&&(t.clearRect(0,0,a.width,a.height),x.slice(0,10).forEach((u,r)=>{const p=(u[0]+2)/4*a.width,S=(u[1]+2)/4*a.height;t.fillStyle=`hsl(${r*36}, 70%, 60%)`,t.beginPath(),t.arc(p,S,5,0,Math.PI*2),t.fill(),x.slice(r+1,10).forEach((n,c)=>{const l=Math.sqrt(Math.pow(u[0]-n[0],2)+Math.pow(u[1]-n[1],2));if(l<1){const o=(n[0]+2)/4*a.width,f=(n[1]+2)/4*a.height;t.strokeStyle=`color-mix(in srgb, var(--glass-white) ${30*(1-l)}%, transparent)`,t.lineWidth=1,t.beginPath(),t.moveTo(p,S),t.lineTo(o,f),t.stroke()}})}))},[x]),Oe=g.useCallback(a=>{He(a),me?.(a),v("select")},[me,v]);g.useEffect(()=>{if(re&&h.trained){const a=setInterval(()=>{N||ge()},5e3);return()=>clearInterval(a)}},[re,h.trained,N,ge]),g.useEffect(()=>{ve()},[x,ve]);const _e=()=>e.jsxs("div",{className:y?"glass-space-y-3":"glass-space-y-4",children:[!y&&e.jsx("h4",{className:"glass-text-sm glass-font-medium glass-text-primary-glass-opacity-80",children:"GAN Models"}),e.jsx("div",{className:`glass-grid ${y?"glass-grid-cols-1 glass-gap-2":"glass-grid-cols-1 md:glass-grid-cols-2 glass-gap-3"}`,style:y?{gridTemplateColumns:"1fr",gap:8}:void 0,children:s.map(a=>e.jsx(j.div,{className:`
