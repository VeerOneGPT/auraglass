...
const result = await page.evaluate(() => {
  ...
  const walk = (el, depth) => {
    ...
    let out = `${'  '.repeat(depth)}<${el.tagName.toLowerCase()} class="${cls.slice(0,130)}" w=${Math.round(b.width)} h=${Math.round(b.height)} bd=${(s.backdropFilter||'none').slice(0,50)}>\n`;
